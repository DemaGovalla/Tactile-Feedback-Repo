...
String dataLabel3 = "Work";



bool label = true; 
float Time = 0, Force, Displacement, Work, Weight=2, CoF;

float R1, R2, curr1, curr2, percent = 0.05;
// Label should be 1 or greater
int threshold = 1024*percent, samples = 30;; //within x% in either direction
...
R2 = analogRead(Resistor2); // Going to A1, blue/green, top of tweezers
   ...
   Force[i] = exp((sqrt(abs(R1-R2)))*0.5);
...
//R1 = analogRead(Resistor1); 
//R2 = analogRead(Resistor2); 
//
//// Calculate Work done
//Force = R1;
//Displacement = R1 - R2;
