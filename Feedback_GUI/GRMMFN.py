...
class ReflexFuzzyNeuroNetwork(object):
    ...
    def __str__(self):
        ...
        # return "Gamma: %s \n Theta: %s \n Number Hyperboxes: %s \n Number OCN_Hyperboxes: %s \n Number CCN_Hyper_boxes: %s " % (
        #                 self.gamma,
        #                 self.theta,
        #                 len(self.HyperBoxes),
        #                 # len(self.OCN_HyperBoxes)
        #                 # len(self.CCN_HyperBoxes)
        #             )
    ...
    def test(self, X, labels):
        ...
        # print(self.query(X[:,0]))
        # print(self.query(X[:,1]))
        # print(self.query(X[:,2]))

        # [results.append(self.query(X[:,i])) for i in range(len(labels))] # for i = 0, ..., 74
    ...
    #     mylist = []
    #     # print("\n this is V",self.V)
    #     # print("\n This is len of V", len(self.V))
    #     for i in range(len(self.V)):
    #         mylist.append([self.MembershipValue(x,self.V[i],self.W[i])])
